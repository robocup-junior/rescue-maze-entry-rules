...
. Points are rewarded for each Successful Victim identification in the field 10 points.
...
. Successful Exit Bonus. The 'exit bonus' condition is satisfied when the robot returns to the starting tile and performs the following: On the starting tile, the robot has to stop and blink (ON: 1s, OFF: 1s) with the green or red LEDs used to identify victims (see Section 4.3, "Build") for at least 10 seconds. If the robot performs an "Exit Performance" on any tile, it must exit the game.
